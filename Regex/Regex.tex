...
For example, the regular expression \ct{'h.*o'} will match the the strings \ct{'ho'}, \ct{'hiho'} and \ct{'hello'}, but it will not match \ct{'hi'} or \ct{'yo'}.
...
\jr{I would rename the set method to initialize... in order to be more intention revealing and follow the smalltalk best practices patterns }
...
The \ct{String} extension method \ct{copyWithRegex:matchesReplacedWith:} does what we want:
...
The operator \ct{+} is like the Kleene star, but it matches exactly \emph{one} or more instances of the regex preceding it. Here it will match the entire substring \ct{'ello'} and replay it in a fresh string with the letter \ct{i}.
...
%:FIGURE -- Regex Syntax in a Nutshell
\begin{table}
\centering
	\begin{tabular}{ll}
		\toprule
...
%:FIGURE -- Regex character classes
\begin{table}[htb]
\centering
...
%:FIGURE -- Primitives to match string boundaries
\begin{table}[htb]
\centering
	\begin{tabular}{lp{8cm}}
		\toprule
...
This chapter presented the Regex package for Squeak. The essential points of this chapter are:
