...
The subsequent step is to create class nodes that will be model browser items. For that purpose, we define two subclasses of \ct{OBNode}:
...
The name \ct{directories} and \ct{files} are also the names of the link between the \ct{Directory} and \ct{File} meta nodes. A more formal explanation is: when one of the two \ct{#directories} and \ct{#files} metaedges is traversed, the name of this metaedge is used as a message name sent to the metanode's node. The two methods are invoked when a directory node is selected.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Enhancing the file browser with actions and icons} \label{sec:enhancing}

A command defines an interaction the user can have with browsed items. All interactions a user can have on the nodes have to be implemented using commands. 
