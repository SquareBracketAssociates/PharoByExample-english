...
This chapter presents the construction of a very simple browser based on the \obf. It also uncover the implementation of the Smalltalk system browser.

\begin{itemize}
\item A browser is implemented by subclassing the class \ct{OBBrowser}
\item All the navigation permitted by the browser is defined with its metagraph.
\item A browser's metagraph is returned by the \ct{defaultMetaNode} class-side method.
\item A metagraph is made of instances of \ct{OBMetaNode} linked each other by sending the message \ct{childAt: aName put: aMetaNode}.
...
\item An action the user can perform is implemented with commands.
...
\item A definition answer \ct{text} to returns a content and \ct{text:} to accept a new content.
