...
\on{It is confusing to talk about the ``\st system browser'' -- in \pharo there is only the OB browser!}
...
A command defines an interaction the user can have with browsed items. All interactions triggered from a menu have to be implemented using commands. We will define a browser command that spawn a file browser on a selected node. \figref{command} depicts the activation of the contextual menu.
...
The variable \ct{requestor} refers to the column that requested for the command. The \ct{target} refers to the node for which the command has been triggered. 
...
The shift key may be added to the keystroke by returning a capital letter instead of a minuscule one. Note that by defining a keystroke, the label is automatically extended with ``\ct{(b)}'' in menus.
...
The command is now fully defined. 

%:===========
\subsubsection{Creating new files}

The command given above remained quite simple: no list to refresh; no interaction with the user; no automatic node selection. We will cover these importing topics by adding a command to manually create files. Browser column will have to be updated with a new entry (the file recently created), the user will have to enter the name of the file to be created, and the new file will be selected after its creation.
...
We want to have the command activate when right clicking either on a directory (to create the file in it), or on a column. This suggest a slightly more elaborated \ct{isActive} method:
...
So far, we've seen only how to create empty files. To modify the content of a file, the browser needs to accept user input from the lower text pane and perform an action on it. 
