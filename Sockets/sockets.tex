...
Modern [applications][applications][application never plural] are often distributed on multiple devices and collaborate through a network to achieve some task.
The basic approach to set up such a collaboration is to use [\textit{sockets}][\textit{sockets}][em is the old latex interface].
A typical use is in the World Wide Web. Browsers and servers interact through HTTP sockets.

The concept of socket was first introduced by researchers from Berkeley University in the 1960's. They defined the first socket API for the C programming language in the context of Unix operating systems. Since then, the concept of socket was spread out to other operating systems. [It's][Its] API was ported to most existing programming languages including Smalltalk.
...
\subsection{Socket}
A remote communication involves at least two applications exchanging some data bytes through a network. Each application accesses the network through at least one socket (see [f][F]igure~\ref{fig:socketConcept}).
...
Note that to use the \ct!NetNameResolver! you need a \sd{working} network connection.
The only exception is for retrieving the local host address. \sd{explain the last sentence}
...
\begin{figure}[h]
\begin{center}
\includegraphics[width=6cm]{addressForName}
\end{center}
\end{figure}
...
This message \sd{which one} returns immediately after issuing to the system (through a primitive call) the request to connect the socket.
...
Now let us build a simple TCP server. A \textit{TCP Server} is an application that awaits TCP connections from TCP clients. Once connection established, both the server and the client can send a receive data in any order. A big difference between the server and the client is that the server uses at least two sockets. \sd{why? so that it can get closed?}
...
\item Accept a client request for connection. As a result,  \textit{socket$_1$} will build a second socket, let's call it \textit{socket$_2$} \sd{why socket1 is creating a second socket and not the server?}.
...
The \ct!backlogSize! is set to 10, meaning that we allocate a buffer for 10 connection requests. This backlog will not be actually used in this example.
...
Now you cannot test this script totally without having another image that will execute the client code. 
