...
We illustrate socket stream use at client side with the following code snipet (\Scrref{script:streamGetLine}).
...
Notice that compared to \scrref{script:closeAndDestroy} (page~\pageref{scr:script:closeAndDestroy}) we skipted here (\Scrref{script:streamGetLine}) one final \ct!String crlf!.
...
SocketStreams can also be used at server side to wrap the interaction socket as shown in \Scrref{script:streamSimpleServer}.
...
The source code presented in this section is available on Squeaksource: http://www.squeaksource.com/PharoInstantMessenge \ben{maybe it should be moved in another repo}. You can load it using Monticello (Chapter \ja{numChapter}). It contains also a user interface that is not explained in this chapter.
...
\begin{script}[script:pimChatMessage]{A Basic Chat application: Class ChatMessage}
...
\begin{script}[script:pimChatServerBehavior]{A Basic Chat application: Class ChatServerBehavior}
...
\begin{script}[script:pimChatServerLauncher]{A Basic Chat application: Class ChatServerLauncher}
...
=======
\begin{classdef}[script:pimChatMessage]{A Basic Chat application: Class ChatMessage}
Object subclass: #ChatMessage
	instanceVariableNames: ''date author message recipient''
	classVariableNames: ''''
	poolDictionaries: ''''
...
\begin{classdef}[script:pimChatServerBehavior]{A Basic Chat application: Class ChatServerBehavior}
Object subclass: #ChatServerBehavior
	instanceVariableNames: ''interactionSocket''
	classVariableNames: ''ConnectedMap''
	poolDictionaries: ''''
...
\begin{classdef}[script:pimChatServerLauncher]{A Basic Chat application: Class ChatServerLauncher}
Object subclass: #ChatServerLauncher
	instanceVariableNames: ''port''
	classVariableNames: ''''
	poolDictionaries: ''''
...
\begin{classdef}[script:pimChatClient]{A Basic Chat application: Class ChatClient}
...
ChatClient class>>example

	(self hostname: ''127.0.0.1'' port: 9999 name: #test) connect
...
An example:
\begin{script}[script:pimServerExample]{A Basic Chat application: Server running}
ChatServerLauncher launch: aPortNumber
\end{script}

\ja{to do}
\begin{script}[script:pimServerExample]{A Basic Chat application: Client running}
