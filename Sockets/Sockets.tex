...
The following snipet show how to send \ct!'Hello from a client'! to a server on the local machine listening on port 9090.
\begin{code}{}
...
But, the string produced by the preceeding \ct!echo! is sent only to the first client to connect.
An alternative solution is to make the \ct!nc! server send text you type.
Simply evalute the following command line. 
\begin{code}{}
echo nc -lk 9090 
\end{code}
...
You can repat these two last actions (type text at the server side, then start client) as many times as needed.
