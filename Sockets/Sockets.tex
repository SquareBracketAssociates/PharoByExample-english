...
interactionStream := SocketStream openConnectionToHostNamed: 'localhost' port: 9999.  
...
Developers should decide a protocol to use and enforce on both interacting sides in order to have correct interaction.

A good practice is to \emph{reify} a protocol, that is to materialize it as an object which wraps a socket stream. 
The protocol object takes analyzes exchanged data and decides accordingly which messages to send to the socket stream.
...
This sequence is considered so useful that the developers of the \ct!SocketStream! class introduced the \ct!sendCommand:! method.
This method (illustrated in \scrref{script:closeAndDestroy}) does append CR and LF after sent data.
When reading CR followed by LF the receiver knows that the received sequence characters is complete and can be safely converted into valid data.
...
Of course, in a real setting, this assumption does not hold and one need to deal with the last part of data that is smaller than a chunk.
