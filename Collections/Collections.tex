...
\begin{itemize}
  ...
  \item {\bf Heterogeneous:}
	...
	An \clsind{Array} will hold any mix of objects, but a \lct{ByteArray} only holds \lct{Byte}s, an \clsind{IntegerArray} only holds \ct{Integer}s and a \clsind{FloatArray} only holds \ct{Float}s.
...
\paragraph{Creation with \mthind{Array class}{with:}} \ct{with:} methods allow one to specify the value of the elements.  
