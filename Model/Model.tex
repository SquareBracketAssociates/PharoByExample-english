...
\noindent
Now we can use this trait in a class that already has its own superclass, for instance the \ct{PBEGame} class that we defined in \charef{firstApp}.
We simply modify the class creation template for \ct{PBEGame} to include a \ct{uses:} keyword argument that specifies that \ct{TAuthor} should be used.
...
If we now instantiate \ct{PBEGame}, it will respond to the \ct{author} message as expected.

\begin{code}{}
PBEGame new author --> 'on'
\end{code}
