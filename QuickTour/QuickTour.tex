...
%:===> CHECK SIZES OF ALL IMAGES FOR LULU LAYOUT
...
The \ind{system browser}\footnote{Also known as the ``Class browser''. \pharo uses the \ind{OmniBrowser} implementation of the system browser, which may also be variously known as ``OB'' or the ``Package browser''.} is one of the key tools used for programming.
...
\dothis{Put the mouse in the system category pane of the system browser and type \short{f}, or select \menu{find class \ldots (f)} by \actclick{ing}.  Type ``time'' in the dialog box and accept it.} 
\noindent
