...
The Lights Out game shown in \figref{gameBoard} is made up of two kinds of objects: the game board itself, and 100 individual cell objects.  The \squeak code to implement the game will contain two classes: one for the game and one for the cells.
...
Type the name of the new category (we will use \scat{BBE-LightsOut}) in the dialog box and click \menu{accept} (or just press the return key); the new category is created, and positioned at the end of the category list.
...
%=================================================================
\section{Defining the class PBECell}
...
\begin{itemize}
  ...
  \item Replace \ct{NameOfSubClass} by \clsind{PBECell}.
  \item Add \ct{mouseAction} to the list of instance variables.
\end{itemize}
The result should look like \clsref{firstClassDef}.

\begin{classdef}[firstClassDef]{Defining the class \ct| PBECell|}
SimpleSwitchMorph subclass: #PBECell
...
This new definition consists of a \st expression that sends a message to the existing class \ct{SimpleSwitchMorph}, asking it to create a subclass called \ct{PBECell}.
(Actually, since \ct{PBECell} does not exist yet, we passed as an argument the \emphind{symbol} \ct{#PBECell} which stands for the name of the class to create.)
...
Once the class definition is accepted, the class will be created and appear in the classes pane of the browser (\figref{PBECell}).
...
\dothis{Type a class comment for \ct{PBECell} and accept it; you can always improve it later.}

\begin{figure}[h!t]
\ifluluelse
	{\centerline {\includegraphics[width=\textwidth]{PBECell}}}
	{\centerline {\includegraphics[scale=0.7]{PBECell}}}
\caption{The newly-created class \ct{PBECell}\label{fig:PBECell}}
\end{figure}
...
\needlines{10}
\begin{numMethod}[scbecellinitialize]{Initializing instances of \ct{PBECell}}
initialize
   super initialize.
   self label: ''.
   self borderWidth: 2.
...
Notice that the method is called \mthind{PBECell}{initialize}.
...
So, when we evaluate \ct{PBECell new}, the message \ct{initialize} will be sent automatically to this newly created object.
...
You can test the effect of the code you have written by creating a new \ct{PBECell} object and inspecting it.

\dothis{Open a workspace. Type the expression \ct{PBECell new} and \menu{inspect it}.}

\begin{figure}[htbp]
   \centering
   \includegraphics[scale=0.7]{PBECellInspector} 
   \caption{The inspector used to examine a PBECell object.\label{fig:PBECellInspector}}
\end{figure}
...
\centering
\ifluluelse
	{\includegraphics[width=\textwidth]{PBECellResize} }
	{\includegraphics[scale=0.7]{PBECellResize} }
...
%=================================================================
\section{Defining the class PBEGame}

Now let's create the other class that we need for the game, which we will call \clsind{PBEGame}.

\dothis{Make the class definition template visible in the browser main window.}
Do this by clicking twice on the name of the already-selected class category, or by displaying the definition of \ct{PBECell} again (by clicking the \button{instance} button.)
Edit the code so that it reads as follows, and \menu{accept} it.

\needlines{6}
\begin{classdef}[sbegame]{Defining the \ct{PBEGame} class}
BorderedMorph subclass: #PBEGame
...
Now let's define an \mthind{PBEGame}{initialize} method for \ct{PBEGame}.

\dothis{Type the following into the browser as a method for \ct{PBEGame} and try to \menu{accept} it:}
...
%\sd{it would be nicer if we would not have to create an instance of PBECell for nothing}
%\on{yes}
...
The next three lines create a new \ct{PBECell} object, and assign its width and height to the appropriate temporary variables. 
...
\paragraph{A typographic convention.} Smalltalkers frequently use the notation ``\verb|>>|'' to identify the class to which a method belongs, so, for example, the \ct{cellsPerSide} method in class \ct{PBEGame} would be referred to as \ct{PBEGame>>cellsPerSide}.
To indicate that this is \emph{not} \st syntax, we will use the special symbol \ct{>>>} instead, so this method will appear in the text as \ct{PBEGame>>>cellsPerSide}
\cmindex{Behavior}{>>}
...
Now let's define the other two methods that are used by the \ct{PBEGame>>>initialize} method. Both of them can go in the \prot{initialization} protocol.

\begin{method}[sbegamecellsperside]{A constant method.}
PBEGame>>>cellsPerSide
   "The number of cells along each side of the game"
   ^ 10
\end{method}
\cmindex{PBEGame}{cellsPerSide}
...
\end{method}
\cmindex{PBEGame}{newCellAt:at:}
%   ^ c      "omit this final line to create a bug"

\dothis{Add the methods \ct{PBEGame>>>cellsPerSide} and \ct{PBEGame>>>newCellAt:at:}.}
Confirm the spelling of the new selectors \ct{toggleNeighboursOfCellAt:at:} and \ct{mouseAction:}.

\Mthref{newCellAt:at:} answers a new PBECell, specialized to position \ct{(i, j)} in the \clsind{Matrix} of cells.
...
PBEGame>>>toggleNeighboursOfCellAt: i at: j
   (i > 1) ifTrue: [ (cells at: i - 1 at: j ) toggleState].
   (i < self cellsPerSide) ifTrue: [ (cells at: i + 1 at: j) toggleState].
   (j > 1) ifTrue: [ (cells at: i  at: j - 1) toggleState].
   (j < self cellsPerSide) ifTrue: [ (cells at: i at: j + 1) toggleState].
\end{method}
\cmindex{PBEGame}{toggleNeighboursOfCellAt:at:}
...
To complete the Lights Out game, we need to define two more methods in class \ct{PBECell} to handle mouse events.
\begin{method}[mouseAction:]{A typical setter method}
PBECell>>>mouseAction: aBlock
   ^ mouseAction := aBlock
\end{method}
\cmindex{PBECell}{mouseAction:}
...
\dothis{Go to the class \ct{PBECell}, define \ct{PBECell>>>mouseAction:} and put it in the \prot{accessing} protocol.}
...
PBECell>>>mouseUp: anEvent
   mouseAction value
\end{method}
\cmindex{PBECell}{mouseUp:}

\on{hm -- differs from version in repository}

\dothis{Add the method \ct{PBECell>>>mouseUp:} and then \menu{categorize all uncategorized} methods.}
\index{method!categorize}

What this method does is to send the message \ct{value} to the object stored in the instance variable \ct{mouseAction}. 
Recall that in \ct{PBEGame>>>newCellAt: i at: j} we assigned the following code fragment to \ct{mouseAction}:
...
\dothis{In a workspace, type \ct{PBEGame new openInWorld} and \menu{do it}.}
...
As depicted in \figref{lightsOutError}, it says \ct{MessageNotUnderstood: PBEGame>>>toggleState}.
...
\ct{PBEGame>>>toggleNeighboursOfCellAt:at:} (near the top).}
...
In this case we can see in the first line of the top panel that the \ct{toggleState} message has been sent to an instance of \ct{PBEGame}, while it should clearly have been an instance of \lct{PBECell}.
The problem is most likely with the initialization of the \ct{cells} matrix.
Browsing the code of \cmind{PBEGame}{initialize} shows that \ct{cells} is filled with the return values of \ct{newCellAt:at:}, but when we look at that method, we see that there is no return statement there!
By default, a method returns \ct{self}, which in the case of \ct{newCellAt:at:} is indeed an instance of \ct{PBEGame}.
\index{method!returning self}

\dothis{Close the debugger window.
Add the expression ``\ct{^ c}'' to the end of the method \ct{PBEGame>>>newCellAt:at:} so that it returns \ct{c}.
...
\dothis{Do: \ct{PBEGame new openInWorld} again.}
...
The simplest way of doing this is by \emph{filing out} the code.  The yellow-button menu in the System Categories pane will give you the option to file out the whole of category \scat{PBE-LightsOut}.
...
\dothis{Yellow-click on the \scat{PBE-LightsOut} category and \menu{fileOut} the contents.}
...
\dothis{Open a fresh \squeak image and use the File List tool to \menu{file in} the PBE-LightsOut.st fileout.
...
What is a package?  For now, you can think of a package as a group of  class and method categories that share the same prefix.  Since we put all of the code for the Lights Out game into the class category called \scat{PBE-LightsOut}, we can refer to it as the \ct{PBE-LightsOut} package.
...
\dothis{Click on the \button{Save} button to save a first version of your Lights Out game on SqueakSource.}
