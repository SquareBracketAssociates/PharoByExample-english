...
Once we have an AST, it is easy to transform an AST to an Intermediary Representation Tree. The class \ct{IRBuilder} offers all the infrastructure to add each possible node. A Visitor walks an AST and builds its corresponding IR tree. \ct{ASTTranslator}  visits each node and for each node  builds the corresponding IR node sequence.
