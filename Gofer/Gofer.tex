...
Monticello defines the package system and version management of Pharo. In Pharo, classes (superclass change, instance variable changes \ldots) and methods (adding, changing, deleting) are elementary entities which are versioned by Monticello. A source is an HTTP server which allows us to save projects (particularly packages) managed by Monticello. This is the equivalent of a forge: It provides the management of contributors and their status, visibility information, a wiki with RSS feed. A source open to everybody is available at \url{http://www.squeaksource.com/}.
...
\sd{we should change the name GoferLinuxMag to something else}
...
\begin{code}{}
Gofer new
    "we work on the project GoferLinuxMag"
    url: 'http://www.squeaksource.com/GoferLinuxMag'	
...
In addition, when gofer does not succeed to load a package in a specified URL, it looks in the local cache which is normally at the root of your image. It is possible to force Gofer not to use the cache using the message \ct{disablePackageCache} or to use it using the message \ct{enbalePackageCache}.
...
Gofer new
    squeaksource: 'GoferLinuxMag';
    version: 'GoferLinuxMag-jannik.laval.2';
    load
\end{code}
...
For example the following code will load the latest version of the package saved by the developer named \ct{jannik.laval}.
...
\subsection{Gofer Actions}
Once a gofer instance is parametrized, we can send it messages to perform differenct actions. 
...
We can load several packages from different servers. To show you a concrete example, you have to load first the configuration of Seaside using its configuration.
...
To work on a local directority, the message \ct{directory:} followed by the absolute path of the directory should be used. Here we specify that the directory to use is reachable at \ct{/home/pharoer/hacking/MCPackages}
...
Since Monticello is a distributed versioning control system, it is often useful to synchronize  versions published on a remote server with the ones locally published in the MC local cache.
...
\paragraph{\ct{commit} and \ct{commit:} operations.}
...
Since it is a good pattern to make sure that we are working on the latest version of a package 
...
\paragraph{\ct{localChanges} and \ct{remoteChanges} operations.}

Before loading or saving a verion, it is often useful to verify the changes 
...
Using the messages \ct{browseLocalChanges} and \ct{browseRemoteChanges}, it is possible to browse the change using a normal code browser.
