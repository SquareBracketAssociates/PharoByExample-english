...
\ja{i cannot load this code in PharoCore 1.3. We need a stable version} \alex{Strange, I just tried and it works with a 13299} 
\begin{code}{}
Gofer new
...
\ja{does it work with RPackage or Categories or both ?}\alex{It works with RPAckage only, there is a ref to RPackageOrganizer}
...
\begin{figure}[htbp]
  ...
  \caption{Two-pane browser. When a package is selected in the left
    pane, the contained classes are shown on the right pane.\alex{This is not Glamour in Pharo}}
...
At this point, we can also add Pane~3 to list the method categories (\figref{classnavigator_wireframe}) \ja{Is it not fig:treeandcategories  (fig 1.4)?}\alex{I do not understand this comment}. The listing
...
\caption{	Improved class navigator including a tree to display the packages and a list of method categories for the selected class.\alex{Why is there visualworks shows in the figure? Where did we get this figure from? This is not Pharo apparently}}
...
\dc{It is currently not clear what are the available ports for each
  presentation. It looks like hover does not exist anymore} \alex{I asked the moose mailing list. We will see}
