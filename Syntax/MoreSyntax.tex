...
% HISTORY:
% 2007-04-06 - split off more syntax
...
The most important messages defined on the class \lct{BlockClosure} are: 
...
\begin{script}{Traditional Array Creation}\eglabel{boring}
| ar | 
ar := Array new: 2. 
...
\seclabel{loopCondition}
Besides conditional expressions, the class \lct{BlockClosure} defines the following methods that implement  iterative conditional control structures: \index{whileTrue}  \index{whileTrue:} \index{whileFalse} \index{whileFalse:} \lct{whileTrue},  \lct{whileTrue:}, \lct{whileFalse}, and  \lct{whileFalse:}.
...
\begin{script}[xxx]{xxx}\eglabel{whileloop}
|counter |
counter := 0.
...
\begin{method}[xxx]{xxx}\mthlabel{while}
...
\begin{script}{factorial?}\eglabel{fact}
| tmp |
tmp := 1.
...
\begin{method}[xxx]{xxx}\mthlabel{lineCount2}
lineCount
...
\begin{method}[xxx]{xxx}\mthlabel{openbrowser}
openBrowser
...
\begin{script}{Capturing an Image}\scrlabel{capt}
| im |
im := Form fromUser.
...
\caption{Rotating and zooming a part of the screen.\figlabel{rotated}}
\end{figure}
...
\begin{script}{Playing with images}\scrlabel{rotate}
