...
\paragraph{Don't get confused!}  In the image side, we have an instance variable which is called ``format'' defined in \ct{Behavior}. It keeps an integer with both, what WE call format plus the amount of variables. What we call format, is the method \ct{instSpec} in the image (which in fact gets the format from the “format” instVar). Finally, the VM agree with us, the method is \ct{formatOf:}  and it refers to what we call format. All in all, the instVar ``forma'' of \ct{Behavior} is misleading. Don’t get confused. \sd{to rewrite a bit later}
