...
%Sake-Packages n'a rien ˆ voir avec MC. Sake-Packages est plus proche d'un systme comme Universes, apt-get ou mac ports. MC a pour r™le de faciliter le dŽveloppement de projets en gŽrant les diffŽrentes versions et les branches. Un systme de package gre quand ˆ lui des distributions qui sont des ensembles de projets inter-dŽpendants. Un systme de package a des fonctionnalitŽs comme l'installation d'un package et des ses dŽpendances, la mise ˆ jour de la liste des packages disponibles (update) ainsi que la mise ˆ jour des packages installŽs (upgrade). Il doit aussi tre capable de dire quels sont les packages installŽs, quels sont ceux qui possdent une mise ˆ jour installable. En thŽorie, il doit aussi tre capable de supprimer un package et tous les packages qui en dŽpendent.
