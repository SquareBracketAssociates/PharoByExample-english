...
\on{Add diagram illustrating Reflection and Reification. Distinguish structural and behavioural reflection. Not just introspection and intercession.
Metaobjects and MO Protocol. Class,  CompiledMethod and MethodDictionary are metaobjects}
...
Although messages can be intercepted by proxies using this technique, one should be aware of the inherent limitations of using a proxy.  In \secref{XXX} we will see another, more general approaches for intercepting messages.
\on{sec ref to be fixed}
...
\on{TestCoverage make a better example -- already in Pharo.
Then mention http://www.squeaksource.com/ObjectsAsMethodsWrap.html as an alternative to MethodWrappers}

\on{MethodWrappers do not use run:with:in: --- much older code. See TestCoverage which uses the newer approach.  MW should be reimplemented.}
...
- Object primitiveChangeClassTo: become: and becomeForward: (see slides with minimal object example)
