...
\begin{code}
ProtoObject subclass: #LoggingProxy	instanceVariableNames: 'subject count'	classVariableNames: ''	poolDictionaries: ''	category: 'PBE-Reflection'
\end{code}
...
We initialize the proxy in the obvious way, and provide an access for the message count:
\begin{code}{}
LoggingProxy class>>>for: anObject
	^ self new initializeSubject: anObject

LoggingProxy>>>initializeSubject: anObject
	subject := anObject
...
\end{code}
We simply intercept all not understood messages, print them to the Transcript, update the message count, and forward the message to the real subject.

\begin{code}{}
proxy := LoggingProxy for: 1@2.proxy count     --> 0proxy + (3@4) --> 4@6proxy count     --> 1
\end{code}

This works nicely in most cases, but there are some shortcomings:
\begin{code}{}
(LoggingProxy for: nil) isNil --> false
\end{code}
It turns out that \ct{ProtoObject} understands \ct{isNil}, so this message is inherited by \ct{LoggingProxy} and not intercepted.  We may implement it in the obvious way:
\begin{code}{}
LoggingProxy>>>isNil	^ self doesNotUnderstand: (Message selector: #isNil)
\end{code}
\noindent
but then we must do this for all methods inherited from ProtoObject.

There is another problem:
\begin{code}{}
(LoggingProxy for: nil) class --> LoggingProxy
\end{code}

The same trick will not work. Curiously, the method \ct{class} is not even implemented in \ct{ProtoObject} but in \ct{Object}, which \ct{LoggingProxy} does not inherit from!
The answer to this riddle is that \ct{class} is never sent as a message but is directly answered by the virtual machine.
...
Although messages can be intercepted by proxies using this technique, one should be aware of the inherent limitations of using a proxy.  In \secref{XXX} \on{to be fixed} we will see another, more general approaches for intercepting messages.

%-----------------------------------------------------------------
\subsection{Generating missing methods}

The second most common application of intercepting not understood messages is to dynamically load or generate the missing methods.
...
\on{Is there a nice anecdote about this?}
