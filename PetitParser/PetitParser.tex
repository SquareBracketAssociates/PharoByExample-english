...
Originally Lukas Renggli have written PetitParser as part of his work
on the Helvetia system. PetitParser is a parsing framework different
...
\ct{PPUnresolvedParser} upfront, because they recursively refer to
each other. The method \ct{#def:} resolves this recursion using the
...
method number in \ct{ExpressionGrammar} as follows:
...
and we factor out the parts for addition (\ct{add}), multiplication
(\ct{mul}), and parenthesis (\ct{parens}) into separate productions.
...
Instantiating the \ct{ExpressionGrammar} gives us an expression parser that returns a default abstract-syntax tree:
...
and we redefine the implementation of \ct{add}, \ct{mul} and
...
subclassing \ct{ExpressionGrammar} and by redefining a few of its
productions:
