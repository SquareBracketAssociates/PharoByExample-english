...
However, when you are first developing a Seaside application, the server will be running locally, and it is easy to use the ordinary \sq development tools in the server image.
...
Seaside components can be instantiated multiple times, and in different contexts. \jr{is the comma correct after times?}
...
You can put Seaside into deployment mode using either the configuration page \ab{How?  I couldn't find this} or the \link{Configure} button in the toolbar.
...
The \link{Form Elements} example illustrates most of the features of forms.
...
\dothis{Go to \url{http://localhost:8080/seaside/tests/alltests} and select \link{Convenience} and select \link{Transaction}.
