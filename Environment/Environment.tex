...
\caption{The \menu{senders (n)} menu item.\label{fig:SendersOfDrawOn}}
\end{minipage}
\hfill
\begin{minipage}[b]{0.24\textwidth}
\centerline {\includegraphics[width=\textwidth]{SendersOfDrawOn2}}
\caption{Choose senders of which message.\label{fig:SendersOfDrawOn2}}
\end{minipage}
\end{figure}
...
%---------------------------------------------------------
\subsection{Packages: declarative categorization of \pharo code}\label{sec:packages}
...
\begin{figure}[btp]
	\begin{center}
	\ifluluelse
		{\includegraphics[width=0.7\textwidth]{SqueakSource-SBE}}
		{\includegraphics[width=0.7\textwidth]{SqueakSource-SBE}}
	\end{center}
	\caption{A Repository browser. \fix}
...
%:===> Fix Monticello snapshots -- NEED WEB ACCESS! <===
\here \on{need web access!}
...
\dothis{Create a package called \scat{PBE-Monticello}, and put a couple of classes in it, as shown in \fix \figref{MCnewcategory}.  Also, create a method in an existing class, such as \ct{Object}, and put it in the same package as your classes, using the rules from page \pageref{sec:packageRules}\,---\,see \figref{MCnewmethod}.}
...
\begin{figure}[tbp]
	\begin{center}
	\ifluluelse
		{\includegraphics[width=0.7\textwidth]{PBE-on}}
		{\includegraphics[width=0.7\textwidth]{PBE-on}}
	\end{center}
...
\begin{figure}[tbp]
	\begin{center}
	\ifluluelse
		{\includegraphics[width=0.8\textwidth]{package-cache-browser}}
		{\includegraphics[width=0.8\textwidth]{package-cache-browser}}
	\end{center}
...
\begin{figure}[btp]
	...
	\end{center}
	\caption{The debugger after stepping \lct{Through} the \ct{do:} method several times. \fix \on{context vars are mixed up}}
...
\begin{figure}[btp]
	...
	\label{fig:methodNamesRandom} % should be *message* names!
\end{figure}
...
%:===> HERE <===
\on{the simple change set browser is gone (hidden) --- just describe the sorter}
...
\begin{figure}[btp]
	...
	\ifluluelse
		...
		{\includegraphics[width=0.7\textwidth]{changeSetBrowser}}
	\end{center}
...
You can see which change set is current and can examine all of the change sets using the  change set browser, available from \menu{World \go Tools \ldots \go Change Sorter}.
...
You can open it from the \menu{World\go{}open \ldots \go{}file list} menu.
