...
selected package\footnote{Recall that packages and categories are not exactly the same thing. See \secref{categoriesPackages}}, but you can change this too if you like.  If you
...
Sometimes you want to open multiple browsers
...
%:===> Rewrite tour of OmniBrowser (once it stabilizes)

%---------------------------------------------------------
\subsection{The button bar}
\label{sec:ButtonBar}

The browser provides several tools for exploring and analysing code. Those tools are most simply accessed from the horizontal \subind{browser}{button bar} at the top of the browser window.  The buttons are labeled \button{implementors},  \button{senders}, \etc
...
The \button{senders} button will give you a list of all methods that may use the selected method. With the browser open on \ct{Morph}, click on the \mthind{Morph}{drawOn:} method in the method pane; the body of \ct{drawOn:} displays in the bottom part of the browser. If you now press the \button{senders} button, a menu will appear with \ct{drawOn:} as the topmost item, and below it, all the messages that \ct{drawOn:} sends (see \figref{SendersOfDrawOn}).  Selecting an item in this menu will open a browser with the list of all methods in the image that send the selected message (\figref{CanvasDraw}).

\begin{figure}[htbp]
	\begin{center}
   \ifluluelse
		{\includegraphics[width=\textwidth]{SendersOfDrawOn}}
		{\includegraphics[scale=0.7]{SendersOfDrawOn}}
	\end{center}
	\caption{A Browser opened on the \lct{Morph} class. Note the horizontal bar of buttons at the top of the browser; here we are using the \lct{senders} button.}
	\label{fig:SendersOfDrawOn}
\end{figure}

\index{browser!implementors button}
The \button{implementors} button works in a similar way, but instead of listing the senders of a message, it lists all of the classes that implement a method with the same selector. To see this, select \lct{drawOn:} in the message pane and then bring up the ``implementors of drawOn:'' browser, either using the \button{implementors} button, or \actclick{ing} in the method pane, or just by typing \short{m} (for {i\textbf{m}ple\textbf{m}entors}) in the method pane with \menu{extent:} selected.  You should get a method list window showing a scrolling list of 88 classes that implement a \ct{extent:} method.
...
% While viewing any one of these methods, try browsing the senders of the \ct{extent:} message:  we found 63 methods that send this message.  
...
The \button{inheritance} button opens a special browser that displays all the methods overridden by the displayed method. 
To see how it works, display the \cmind{ImageMorph}{drawOn:} method.
Note the triangular icons next to the method name (\figref{OBinheritanceBrowser}).
The upward-pointing triangle tells you that \ct{ImageMorph>>>drawOn:} overrides in inherited method (\ie \ct{Morph>>>frawOn:}), and the downward-pointing triangle tells you that it is overridden by subclasses. (You can click on the icons to navigate to these methods.)
Now click on the \button{inheritance} button.
...
%:===> Describe package/hierarchy toggle?
...
\index{browser!variables button}
The \button{variables} button helps you find where an instance variable or a class variable is used; the same information is accessible by \actclick{ing} in the class pane and selecting\menu{chase variables}.
You will then be presented with a \emph{chasing browser} that will allow you to walk through the accessors of all instance variables and class variables, and, in turn, methods that send these accessors, and so on.

\begin{figure}[btp]
	\begin{center}
	\ifluluelse
		{\includegraphics[width=\textwidth]{chasingBrowser}}
		{\includegraphics[scale=0.7]{chasingBrowser}}
	\end{center}
	\caption{A chasing browser for \ct{Morph}.}
	\label{fig:chasingBrowser}
\end{figure}
...
%:===> View ... button to disappear? (context menu only)

\index{browser!view button}
The \button{view \ldots} button brings up the ``what to show'' menu, which allows you to choose what the browser shows in the source pane.  Options include the \menu{source} code, \menu{prettyPrint}ed source code, \menu{byteCode} and source code \menu{decompile}d from the byte codes.
...
\begin{figure}[btp]
	...
	\ifluluelse
		...
		{\includegraphics[scale=0.7]{refactoring}}
	\end{center}
...
For example, the \ct{ScaleMorph} class belongs to the \scat{Morphic-Widgets} category. \fix
...
\dothis{Create a category \fix called \scat{PBE-Monticello}, and put a couple of classes in it, as shown in \figref{MCnewcategory}.  Also, create a method in an existing class, and put it in the same package as your classes, using the rules from page \pageref{sec:packageRules}\,---\,see \figref{MCnewmethod}.}
