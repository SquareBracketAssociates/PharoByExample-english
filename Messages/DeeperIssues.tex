...
% HISTORY:
% 2007-06-11 - split off from main chapter
...
\caption{Another example of using parentheses to control the way messages are executed.\figlabel{compoBinBracket}}
\end{figure}

\begin{figure}[ht]
% MISSING FIGURE
% \centerline{\includegraphics[width=5cm]{ucompoBinNoBracket}} 
\caption{As the message \ct{@} when sent to a \textit{point} returns a B3DVector, the expression does not return a simple point but a B3DPoint.  \figlabel{compoBinNoBracket}}
\end{figure}
...
\begin{script}{Examples of long keyword based messages}\scrlabel{longkey}
...
\begin{script}{Identifying keyword-based messages}\scrlabel{keywordComplex}
1   | dist| 
...
\begin{script}{Identifying keyword-based messages}\scrlabel{keywordComplex2}
1   | dist| 
...
\begin{script}{Conditional Requiring Parenthesis}\scrlabel{withParenthesis2}
| aRect aTurtle |
aTurtle := Turtle new.
