...
\noindent
Have you ever had this problem when trying to load a project: you get an error because a package that you were not even aware of is missing, or worse, it is present, but you have the wrong version? You are probably nodding. This problem probably occurred because the project loaded fine for the  developers, but only because they were working in a context that was different from yours. 

The solution is for the project developers to use a \emph{package management system} to explicitly manage the dependencies between packages.  
...
Indeed, package management systems are sometimes incorrectly referred to as installers. This can lead to confusion, because as package managment system does a lot more than install software.  You may have used package management systems in other contexts: examples include Envy (in VisualAge Smalltalk), Maven (in Java), and apt-get/aptitude (in Debian and Ubuntu).

One of the key features of good package management is that it should \emph{correctly load any package}: you should never need to manually install anything.  To make this possible, each dependency, and the dependencies of the dependencies, and so on, must be specified in the description of the package, with enough information to allow the package management tools to load them in the correct order. 
...
\begin{description}
\item[\textit{Declarative modeling.}] A Metacello project has named versions consisting of lists of Monticello package \emph{versions}. Dependencies are explicitly expressed in terms of named versions of required projects. A \emph{required project} is a reference to another Metacello project.

\item[\textit{ Distributed repositories.}] Metacello project metadata is represented as instance methods in a class.  This means that Metacello project metadata can itself be stored as a Monticello package.  This means that Metacello project metadata is subject to version control.  As a result, it is easy for distributed groups of developers to collaborate on ad-hoc projects.
...
\ab{This is the first time that you have used the term `Configuraton'.  It needs to be explained first}.
...
ConfigurationOfCoolToolSet>>baseline02: spec 						!\emph{"could be called differently just a convention"}!
       <version: '0.2-baseline'> 						                        !\emph{"pragma and -baseline are important"}!
       ...
       spec for: !\textbf{\#common}! do: [                                    !\emph{"\#common/\#pharo/\#gemstone/\#pharo'1.4'"}!     
              ...
              spec !\textbf{project:}! 'CoolBrowser default' !\textbf{with:}! [
                            spec
                                   className: 'ConfigurationOfCoolBrowser';
                                   versionString: '1.0';			   !\emph{'1.0' ?? because baseline so ?}!
             ...
             !\emph{"Our internal package dependencies"}!
              ...
              !\emph{"No idea :)"}!                             
              spec for: #CBPresent do: [ spec group: 'default' with: #('CoolToolSet-CB')].  
              	!\emph{what is CBPresent a class?}!
              spec for: #CBNotPresent do: [  
...
ConfigurationOfCoolBrowser>>version07: spec 					!\emph{"could be called differently just a convention"}!
       <version: '0.7' imports: #('0.7-baseline')>				!\emph{"pragma: no baseline so this is version"}!
															!\emph{"do not import baseline from other baselines"}!
    
       spec for: !\textbf{\#common}! do: [                            !\emph{"\#common/\#pharo/\#gemstone/\#pharo'1.4'"}! 
              spec blessing: #release.								!\emph{"WHAT IS THAT? \#stable?/\#bleedingEdge"}!
