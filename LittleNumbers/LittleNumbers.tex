...
Due to the fact that \ct{1 - 0 = 1} and \ct{1 - 1 = 0}, taking the complement of each digit is exactly the same as flipping 1's to 0's and vice versa and adding 1.
...
\ct{-69} is represented on 8 a bit encoding as: \ct{1011 1011}. So get the value out of the bit representation is simple we sum: $-2^7 + 0*2^6 + 2^5 + 2^4 + 2^3 + 0*2^2 + 2^1 + 2^0$, \ie \ct{-128 + 32 + 16 + 8 + 2 + 1} and we get -69. 




\paragraph{Subtracting.}
When we want to compute \ct{110110 - 101}, we will compute the 2's completement and add it. 
So we will add 0000000000000000000000000110110 and 1111111111111111111111111111011, and get 0000000000000000000000000110001. And we get well \ct{54-5 = 49}.
