...
Smalltalk has a format for representing number in different bases. We write \ct{2r1101} where 2 indicates the base or radix, here 2, and the rest the number expressed in this base.
...
\mmp{show an example, otherwise it is difficult to understand. Use for example 2r000001101 bitAt: 5 -> returns 0. Do the drawing you always do and it will be much clear. }


Pharo offers the traditional Boolean operations for bit sequence. Hence the messages \ct{bitAnd:}, \ct{bitOr:}, and \ct{bitXor:} can be sent to numbers. 
...
\mmp{Write it as a math formula, otherwise it is difficult to understand the precedense of the calculus. Is it  10 raisedTo: (k-n)  or is it (10 raisedTo: k) - n  ? }

\mmp{Apart from that, what do you mean by "where $k$ is the number of digits in the decimal representation of $n$."  ?  For the example of 1968 I would have expected 4 since it has 4 digits. But instead it is 5. I am so stupid? }
...
\mmp{what is to drop the carry?  how much do you drop and how did you know that? how did you get 5? }

Now $98-60$. The 10's complement of 60 is $9-6$, $9-0$ \ie $39 + 1$ \ie 40. 
$98-60 = 98 + 40 - 100 = 138 - 100 = 38$.

\mmp{You said that ``The key point of complement techniques is to convert subtractions into additions''. But here there is a substraction as well, the minus 100. So?}
...
\mmp{I was a little bit lost with this example. Are you sure all the numbers are correct?}
...
\mmp{if we see those prints, we can notice that they are 31 bits long instead of 32. Maybe just saying in one line that the last bit is used to differenciate memory addresses from immediate small integers helps}
...
\mmp{why you took -2 raisedTo: 7 as the negative?  you should explain which bit marks the sign}
...
\mmp{I would explicitly mention the fact that the most significant bit is used for the sign. Otherwise, one would expect that 11111111 is 255 instead of -1}
...
\mmp{explain why 31 bits and not 32 hahah}
