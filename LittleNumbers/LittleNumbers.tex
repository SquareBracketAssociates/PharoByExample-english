...
The message \ct{bitShift:} is equivalent to \ct{>>} and \ct{<<}. It just use negative and positive integers to indicate the shift direction. A positive argument offers the same behavior than \ct{<<}, multiplying the receiver by a power of 2. A negative is similar to \ct{>>}. 
...
Smalltalk offers way to access bit information. The message \ct{bitAt:} returns the value of the bit at a given position. It follows the Smalltalk convention that collection starts one. 
...
Pharo offers the traditional Boolean operations for bit sequence. Hence the messages \ct{bitAnd:}, \ct{bitOr:}, and \ct{bitXor:} can be send to numbers. 
...
Using the rule two we compute $9-1$, $9-9$, $9-6$, $10-8$ \ie 8032. So our 10's complement is 8032. Indeed $1968 + 8032 = 10000 = 10^{5}$. Therefore it follows well the definition above: 8032 is the result of $10000 - 1968$.
