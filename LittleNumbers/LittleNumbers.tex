...
\sd{mmp: show an example, otherwise it is difficult to understand. Use for example 2r000001101 bitAt: 5 -> returns 0. Do the drawing you always do and it will be much clear. }
...
\sd{mmp: Write it as a math formula, otherwise it is difficult to understand the precedense of the calculus. Is it  10 raisedTo: (k-n)  or is it (10 raisedTo: k) - n  ? }

\sd{mmp: Apart from that, what do you mean by "where $k$ is the number of digits in the decimal representation of $n$."  ?  For the example of 1968 I would have expected 4 since it has 4 digits. But instead it is 5. I am so stupid? }
...
\sd{mmp: what is to drop the carry?  how much do you drop and how did you know that? how did you get 5? }

Now $98-60$. The 10's complement of 60 is $9-6$, $9-0$ \ie $39 + 1$ \ie 40. 
$98-60 = 98 + 40 - 100 = 138 - 100 = 38$.

\sd{mmp: You said that ``The key point of complement techniques is to convert subtractions into additions''. But here there is a substraction as well, the minus 100. So?}
...
\sd{mmp: I was a little bit lost with this example. Are you sure all the numbers are correct?}
...
\sd{mmp: what is the ``bitwise NOT''?}
...
\sd{mmp if we see those prints, we can notice that they are 31 bits long instead of 32. Maybe just saying in one line that the last bit is used to differentiate memory addresses from immediate small integers helps}
...
\sd{mmp: why you took -2 raisedTo: 7 as the negative?  you should explain which bit marks the sign}
