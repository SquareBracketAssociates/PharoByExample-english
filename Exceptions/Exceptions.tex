...
\noindent
The \lct{deprecated: \emph{anExplanationString} on: \emph{date} in: \emph{version}} message allows more information about the deprecation occurred to be specified.
...
But in fact it too is trivial, because primitive 199 \emph{always} fails\footnote{The source code of the primitive is \ct{Interpreter>>>primitiveMarkHandlerMethod} in the \pkgind{VMMaker} \sqsrc project}. 
...
So, if no action is taken in response to a \ct{Notification}, the program can continue to execute. 
...
More often, it is only safe to resume an exception if the state of he system is modified in some way before the exception is resumed.
So, if you declare a resumable exception, you should be very clear about what you expect an exception handler to do before it resumes the exception.
...
\lct{\emph{aDictionary} at: \emph{aKey}} will signal an \ct{Error} if \ct{aKey} is not present.
But you should not write a handler for this error!
If the logic of your application is such that there is some possibility that \ct{aKey} will not be in the dictionary, then you should instead use \lct{at: \emph{aKey} ifAbsent: [\emph{remedial action}]}.
In fact, \ct{Dictionary>>>at:} is implemented using \ct{Dictionary>>>at:ifAbsent:}.

When you write methods that signal exceptions, you should consider whether you should also provide an alternative method that takes as an additional argument a remedial block to be executed if the normal action cannot be completed.  
