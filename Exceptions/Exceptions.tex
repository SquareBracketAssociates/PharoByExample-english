...
In \st it is possible to set a breakpoint within a method by sending the message \ct{self halt}. The method \ct{halt}, implemented in \ct{Object}, makes use of exceptions to open a debugger at the location of the breakpoint.
...
The ANSI standard is not clear regarding the difference between \ct{do: [:ex | 100 ]} and \ct{do: [:ex | ex return: 100]} to return a value. We suggest you to use \ct{return:} since it is more intention-revealing, even if these two expressions are equivalent in \pharo.
...
%:===> CONVERT USER INTERACTION TO REAL EXAMPLE <===
...
Up until now, we presented the use of exceptions in \st and their power without really saying a word on their implementation
...
%:===> CANCEL ERROR AND NOTIFICATION SECTION ? <===
