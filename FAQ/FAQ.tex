...
\end{faq}
\answer
You are probably using an image in which a different version of the \ind{OmniBrowser} is installed as the default browser.  In this book we assume that the OmniBrowser \emph{Package Browser} is installed as the default.  You can change the default by clicking on the menu bar of the browser.

%:===> CHECK/FIX instructions for switching default browser

%  You can change the default by clicking on the little menu icon in the top-left corner of the browser window, between the ``\textsf{X}'' and the browser label, and selecting ``Choose new default browser'' from the menu that appears. Select \ct{#Browser} to get the plain old browser.
