...
Generally you won't handle the mantissa \sd{nicolas what do you want to say here }, but rather the mantissa shifted 52 bits to the left in order to operate on exact integers. The result includes the leading 1 and should thus be 53 bits long for a normal number (that's the float precision):
 \begin{code}{}
...
Let us see in detail how we could get the fractional bits of \ct{1/10} \ie \ct{2r1 / 2r101}. For that, we must lay out the division:
...
But that's not all the story... Let us inspect the above fraction: \sd{explain what is the lowBit and highBit}
 \begin{code}{}
